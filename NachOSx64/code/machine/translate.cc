...
#include "addrspace.h"
#include "system.h"
#include "../userprog/invertedpagetables.h"	// To fix invalid use of incomplete type ‘class invertedpagetable’
...
// check for alignment errors
	...
	// if(tlb == NULL) {
	// 	std::cout << "tlb is NULL" << std::endl;
	// }
	// if(pageTable == NULL) {
	// 	std::cout << "pageTable is NULL" << std::endl;
	// }
	...
	entry = &pageTable[vpn];
        ...
        for (entry = NULL, i = 0; i < TLBSize; i++) {
    	    if (tlb[i].valid && (tlb[i].virtualPage == (int)vpn)) {
				...
				#ifdef VM
				// std::cout << "update inverted page table" << std::endl;
				invertedPageTable->updatePage(tlb[i].physicalPage, false); // update the inverted page table uses for given page
				#endif
				break;
	    	}
		}
